...
final eventDatesProvider = FutureProvider.family<List<DateTime>, DateTime>((ref, month) async {
  ...
  try {
    ...
    for (final rehearsal in rehearsals) {
      final startDate = DateTime.fromMillisecondsSinceEpoch(rehearsal.startsAtUtc, isUtc: true);
...
final availabilityMapProvider = FutureProvider.family<Map<DateTime, AvailabilityStatus>, DateTime>((ref, month) async {
  ...
  try {
    ...
    for (final item in availabilityItems) {
      final date = DateTime.fromMillisecondsSinceEpoch(item.dateUtc, isUtc: true);
